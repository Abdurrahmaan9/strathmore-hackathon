...
const OverviewStats: React.FC<OverviewStatsProps> = ({ candidates }) => {
  ...
  console.log('XXXX', totalCandidates)
  ...
  const highRiskCandidates = candidates.filter(
    candidate => candidate.integrity?.risk_level === 'RED'
  ).length
...
React.useEffect(() => {
    ...
    fetch(`/api/proxy/${API_ENDPOINTS.donors.getTotal}`)
        .then(res => res.json())
        .then(data => {
            console.log('🔍 DEBUG - Raw donors response:', data);
            console.log('🔍 DEBUG - Total donors value:', data?.total_donors);
            setTotalDonors(data?.total_donors ?? 0);
        })
        .catch(err => console.error('Failed to fetch total donors:', err));
}, []);
  ...
  const avgIntegrityScore =
    candidates.length > 0
      ? Math.round(
          candidates.reduce(
            (sum, candidate) => sum + (candidate.integrity?.score || 0),
            0
          ) / candidates.length
        )
      : 0
  ...
  const suspiciousActivities = candidates.filter(
    candidate =>
      candidate.integrity?.risk_level === 'RED' ||
      (candidate.financial_summary?.total_digital_spend || 0) > 1000000
  ).length
