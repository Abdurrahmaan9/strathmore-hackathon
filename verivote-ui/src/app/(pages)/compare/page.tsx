...
// Updated scoring interfaces based on API documentation
...
interface ComparisonData {
    ...
    // Updated suitability scores structure based on API documentation
    suitability_scores: {
...
const ComparePage: React.FC = () => {
    ...
    const [expandedSections, setExpandedSections] = useState<{ [key: string]: boolean }>({
        ...
        endorsements: false,
        finance: false,
        scoring_breakdown: false
    });

    // Filter options
    const [counties, setCounties] = useState<FilterOption[]>([]);
    ...
    useEffect(() => {
        ...
        const candidate1Slug = searchParams?.get('candidate1');
        const positionParam:any = searchParams?.get('position');

        if (candidate1Slug) {
            // Set position filter if provided
            if (positionParam) {
                setFilters(prev => ({
                    ...prev,
            ...
            // Load the candidate data and preselect
            loadCandidateFromUrl(candidate1Slug, positionParam);
        }
    ...
    const CandidateCard: React.FC<{ candidate: ComparisonCandidate; position: 'left' | 'right' }> = ({ candidate, position }) => (
        <div className={`text-center ${position === 'left' ? 'pr-4' : 'pl-4'}`}>
            ...
            <p className="text-sm text-gray-600 mb-1">{candidate.party.name}</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className={`p-6 rounded-lg border-2 ${winner === 1 ? 'border-green-400 bg-green-50' : 'border-gray-200 bg-white'}`}>
                        <div className="text-center">
                            ...
                            <div className="text-sm text-gray-600 mb-3">{candidate1Scoring.grade.description}</div>
                    ...
                    <div className={`p-6 rounded-lg border-2 ${winner === 2 ? 'border-green-400 bg-green-50' : 'border-gray-200 bg-white'}`}>
                        <div className="text-center">
                            ...
                            <div className="text-sm text-gray-600 mb-3">{candidate2Scoring.grade.description}</div>
                                                    ...
                                                    return (
                                                        <div key={category.id} className="bg-white p-3 rounded border">
                                                            ...
                                                            <div className="flex justify-between text-sm">
                                                                <div className={winner === 1 ? 'font-bold text-green-600' : 'text-gray-600'}>
                                                                    {selectedCandidates[0]?.name}: {category.score.toFixed(1)}
                                                                </div>
                                                                <div className={winner === 2 ? 'font-bold text-green-600' : 'text-gray-600'}>
